...
export function strftime(date: Date, format: string): string {
  ...
  const monthFormatterLong = new Intl.DateTimeFormat(undefined, {
    month: "long",
  })
  const monthFormatterShort = new Intl.DateTimeFormat(undefined, {
    month: "short",
  })
...
export function replace(
  ...
  const pattern =
    oldvalue.length === 0
      ? new RegExp("(?=)", "gu")
      : new RegExp(escapeRegExp(oldvalue), "gu")
